...
@router.get('/get_rewards', dependencies=[Depends(RightsChecker([10, 11]))])
def get_rewards(token_payload: any = Depends(get_current_user)):
    try:
        dataset = data_access.get_rewards()
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': data_frame_to_json_object(ds)}
...
@router.post('/get_reward_qualification_details', dependencies=[Depends(RightsChecker([10, 11]))])
def get_reward_qualification_details(req: GetRankDetails_Request, token_payload: any = Depends(get_current_user)):
    try:
        if token_payload["role"] == 'User':
            req.user_id = token_payload["user_id"]

        dataset = data_access.get_reward_details(req=req)
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': data_frame_to_json_object(ds),
...
@router.post('/get_hong_kong_qualifiers', dependencies=[Depends(RightsChecker([10, 11]))])
def get_hong_kong_qualifiers(req: GetRankDetails_Request, token_payload: any = Depends(get_current_user)):
    try:
        if token_payload["role"] == 'User':
            req.user_id = token_payload["user_id"]

        dataset = data_access.get_hong_kong_qualifiers(req=req)
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': data_frame_to_json_object(ds),
...
@router.get('/get_hong_kong_qualification_details', dependencies=[Depends(RightsChecker([11]))])
def get_hong_kong_qualification_details(user_id: str, token_payload: any = Depends(get_current_user)):
    try:
        dataset = data_access.get_hong_kong_qualification_details(user_id=user_id)
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': data_frame_to_json_object(ds)}
...
@router.post('/get_dubai_qualifiers', dependencies=[Depends(RightsChecker([10, 11]))])
def get_dubai_qualifiers(req: GetRankDetails_Request, token_payload: any = Depends(get_current_user)):
    try:
        if token_payload["role"] == 'User':
            req.user_id = token_payload["user_id"]

        dataset = data_access.get_dubai_qualifiers(req=req)
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': data_frame_to_json_object(ds),
