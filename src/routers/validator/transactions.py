...
@router.get('/get_polygon_validator_transactions', dependencies=[Depends(RightsChecker([10, 11]))])
def get_polygon_validator_transactions(page_index: int = 0, page_size: int = 10, token_payload: any = Depends(get_current_user)):
    try:
        dataset = data_access.get_polygon_validator_transactions(page_index=page_index, page_size=page_size)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': data_frame_to_json_object(ds),
                    'data_count': int(dataset['rs1'].iloc[0].loc["total_records"])}
