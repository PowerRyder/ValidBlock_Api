...
@router.get('/update_validator_package_discount_percentage', dependencies=[Depends(RightsChecker([11]))])
...
@router.get('/update_coin_package_discount_percentage', dependencies=[Depends(RightsChecker([11]))])
async def update_coin_package_discount_percentage(percentage: float, token_payload: any = Depends(get_current_user)):
    try:
        dataset = data_access.update_coin_package_discount_percentage(percentage=percentage, by_user_id=token_payload["user_id"])
        # print(dataset)
        if len(dataset) > 0 and len(dataset['rs']):
            ds = dataset['rs']
            if ds.iloc[0].loc["success"]:
