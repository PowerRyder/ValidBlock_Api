...
def get_validator_roi_income(req: GetRoiIncome_Request, match_exact_user_id: bool = False):
    res = execute_query("call usp_get_validator_roi_income(_user_id => %s, _match_exact_user_id => %s, _on_date => %s::timestamptz[], _package_id => %s, _page_index => %s, _page_size => %s)",
...
def get_validator_roi_level_income(req: GetROILevelIncome_Request, match_exact_user_id: bool = False):
    res = execute_query("call usp_get_validator_roi_level_income(_user_id => %s, _match_exact_user_id => %s, _downline_id => %s, _level => %s, _on_date => %s::timestamptz[], _package_id => %s, _roi_id => %s, _page_index => %s, _page_size => %s)",
